...
//			if(result2DWithSign < 0 && Math.acos(r) > 0)	return -(Math.acos(r));
	...
	public static double[][] getRotationMatrixAboutXAxis(double rotAngle){
		...
		matrix[1][0] = 0; matrix[1][1] = Math.cos(rotAngle); matrix[1][2] = -Math.sin(rotAngle);
		matrix[2][0] = 0; matrix[2][1] = Math.sin(rotAngle); matrix[2][2] = Math.cos(rotAngle);
		return matrix;
	}
	...
	public static double[][] getRotationMatrixAboutZAxis(double rotAngle){
		double[][] matrix = new double[3][3];
		matrix[0][0] = Math.cos(rotAngle); matrix[0][1] = -Math.sin(rotAngle); matrix[0][2] = 0;
		matrix[1][0] = Math.sin(rotAngle); matrix[1][1] = Math.cos(rotAngle); matrix[1][2] = 0;
